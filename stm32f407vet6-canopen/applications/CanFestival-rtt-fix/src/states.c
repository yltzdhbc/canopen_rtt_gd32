...
void canDispatch(CO_Data* d, Message *m)
{
	 ...
	 switch(cob_id >> 7)
	{
	    ...
	    case TIME_STAMP:
	        SillySlave_obj1012 = 0x0;
	        DAY = 0;
	        SillySlave_obj1012 = m->data[0];
	        SillySlave_obj1012 += m->data[1] * 0x100;
	        SillySlave_obj1012 += m->data[2] * 0x10000;
	        SillySlave_obj1012 += m->data[3] * 0x1000000;
	        DAY = m->data[4] + m->data[5] * 0x100;
	        MS = SillySlave_obj1012;
	        break;
		case PDO1tx:
		case PDO1rx:
		case PDO2tx:
		case PDO2rx:
		{
		    uint8_t nodeid = m->cob_id & 0x7F;
		    if (m->data[0] == 0x01 && d->nodeState == Operational) ///>开启传输 确认ID号。
		    {
		        adc_init();
		        SillySlave_obj1800_Transmission_Type = 0x0A;
		    }
		    else if (m->data[0] == 0x0F && *d->bDeviceNodeId == nodeid && d->nodeState == Operational) ///>电磁阀控制命令
		    {
		        IO_Set(d->canHandle,m);
		    }
		}
		case PDO3tx:
		case PDO3rx:
